...
retry:
	...
	var faviconMMH3, faviconPath, IconContent string
	if scanopts.Favicon {
		var err error
		faviconMMH3, faviconPath, err, IconContent = r.handleFaviconHash(hp, req, resp)
		if err == nil {
			builder.WriteString(" [")
			if !scanopts.OutputWithNoColor {
...
func (r *Runner) handleFaviconHash(hp *httpx.HTTPX, req *retryablehttp.Request, currentResp *httpx.Response) (string, string, error, string) {
	...
	if path.Ext(req.URL.Path) == ".ico" {
		...
		encodedData := ""
		if err == nil {
			encodedData = base64.StdEncoding.EncodeToString(currentResp.Data)
		}
		return hash, req.URL.Path, err, encodedData
	}
	...
	encodedData2 := ""
	if err == nil {
		encodedData2 = base64.StdEncoding.EncodeToString(resp.Data)
	}
	return hash, req.URL.Path, err, encodedData2
}
