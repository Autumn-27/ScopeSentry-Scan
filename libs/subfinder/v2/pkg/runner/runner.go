...
func NewRunner(options *Options) (*Runner, error) {
	...
	// Otherwise load the default provider system
	if fileutil.FileExists(options.ProviderConfig) {
		//gologger.Info().Msgf("Loading provider system from %s", options.ProviderConfig)
		options.loadProvidersFrom(options.ProviderConfig)
	} else {
		//gologger.Info().Msgf("Loading provider system from the default location: %s", defaultProviderConfigLocation)
